/**
 * Submit a previously signed PLT burn (from signed-burn.json).
...
(async () => {
    console.log('Using object+signature path (v10.0.2)');

    let raw: any;
    try {
        raw = JSON.parse(readFileSync('signed-send.json', 'utf8'));
        console.log('✅ Loaded signed transaction from signed-burn.json');
    } catch (error) {
        console.error('❌ Failed to read signed-burn.json:', error);
        throw error;
    }
